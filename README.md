...
The publication investigates 2D and  3D mapping tools within ArcGIS. The paper focuses on the specifically on petroleum reserved mapping. The writer focuses on explaining the uses of 3D modeling in mining applications. These uses include mine planning, engineering, and health and safety to name a few. The ArcGIS functions focus on the use of point, line, polygon features for 2D mapping and solid features to accomplish the 3D mapping.  The point features can be used to help define boundaries in a 2D range.  The line features are a collection of points that can be pieced together. The polygon feature allows for lines to be connect to form two dimensional shapes to define regions, boundaries, and other interactions. The solid feature allows for a 3rd dimension to be added, but with great complication. ArcGIS has a multipatch feature that simplifies the process allowing for easy creation of 3D shapes. 

In my application ArcGIS is useful for creating maps of planned drill holes. This can be through a 2D hole collars. It is also useful to created lines that represent depths of the drillholes. In a 3D shape looking a certain type of rock thicknesses to get to the desired rock type can be useful. 
